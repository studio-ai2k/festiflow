...
def fetch_shotgun_tickets(shotgun_event_id, token, organizer_id, event_days=None):
    ...
    while url:
        ...
        # Rate limit: Shotgun allows 100 req/min. Pace at ~80 req/min.
        if page > 1:
            time.sleep(0.8)
        ...
        if resp is None:
            status = 'partial' if all_tickets else 'error'
            return all_tickets, {
                'status': status,
                'error': f'API request failed on page {page} after retries',
...
def fetch_dice_tickets(dice_event_id, token, event_days=None):
    ...
    dice_event_id: numeric ID from config (e.g. '540197').
    Automatically encoded to Relay ID format (Event:540197 → Base64).
    ...
    # Convert numeric ID to DICE GraphQL Relay ID
    # e.g. '540197' → 'Event:540197' → base64 → 'RXZlbnQ6NTQwMTk3'
    raw_id = str(dice_event_id).strip()
    if raw_id.startswith('RXZlbnQ6'):
        # Already a Relay ID
        relay_id = raw_id
    else:
        relay_id = base64.b64encode(f'Event:{raw_id}'.encode()).decode()

    log(f"Fetching DICE tickets for event {raw_id} (relay: {relay_id})...")
    started = datetime.utcnow()
    all_tickets = []
    errors = []
    null_price_count = 0
    ...
    if not event_node:
        return [], {
            'status': 'error',
            'error': f'Event {raw_id} not found',
            'tickets_fetched': 0,
            'event_id': raw_id,
            'fetched_at': datetime.utcnow().isoformat() + 'Z'
        }
    ...
    while True:
        ...
        variables = {'eventId': relay_id, 'first': 50}
        if cursor:
